        ...
        body: BlocConsumer<AdminHomeCubit, AdminHomeState>(
          ...
          builder: (context, state) {
            return state.when(
              ...
              success: (data) {
                final list =
                    (data as HomeAdminResponse).data?.notRespondedBookings?.map((toElement)=>toElement.property).toList() ?? [];
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text(property?.city?.nameEn??""),
                        trailing: const Icon(
                          Icons.arrow_forward,
                          color: Colors.white,
                        ),
                        onTap: () {
                          print("MAZ CONTRACT ${list[index]!.contract}");
                          context.pushNamed(Routes.lawyerDetailsScreen,arguments: PropertyModelMapper().getAdmin(list[index]!));
                        },
                      ),
                    );
                  },
